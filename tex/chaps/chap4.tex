...
\paragraph{}The data on survival probability for all ages has been taken from Turkish Statistical Institute's (TUIK) database. Obviously, we assume that a person is alive at 28 years with $100\%$ probability. All of these findings have been summarized in Table 4.1. 
...
where the latter two are Anadolu Hayat's aggressive and moderate investment options. Since we are only interested in age span between 28 and 57, Figure 4.8 shows the risky asset share $\pi_t$ only for that interval. 
